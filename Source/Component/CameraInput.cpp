...
void CameraInput::OnInputUpdate(float deltaTime, int mods)
{
	...
	/*if (window->KeyHold(GLFW_KEY_W))	camera->MoveForward(deltaTime * 5);
	if (window->KeyHold(GLFW_KEY_S))	camera->MoveBackward(deltaTime * 5);
	if (window->KeyHold(GLFW_KEY_A))	camera->MoveLeft(deltaTime * 5);
	if (window->KeyHold(GLFW_KEY_D))	camera->MoveRight(deltaTime * 5);
